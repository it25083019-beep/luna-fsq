...
def test_catalog_has_luna_and_animals():
    ...
    assert (get_companion("luna").get("voice") or {}).get("clone_wav", "").endswith("luna-ref.wav")
    assert (get_companion("luno").get("voice") or {}).get("clone_wav", "").endswith("luno-ref.wav")
    assert (get_companion("ren").get("voice") or {}).get("browser_pitch", 1) < 0.7
    assert (get_companion("hachi").get("voice") or {}).get("browser_pitch", 1) >= 1.3
    assert (get_companion("momo").get("voice") or {}).get("browser_pitch", 1) >= 1.25
    assert (get_companion("taro").get("voice") or {}).get("browser_pitch", 1) <= 0.85
    ...
    assert (audio / "luna-ref.wav").is_file()
    assert (audio / "luno-ref.wav").is_file()
    assert (audio / "luna-ref.wav").read_bytes()[:4] == b"RIFF"
    assert (audio / "luno-ref.wav").read_bytes()[:4] == b"RIFF"
    owner = Path(__file__).resolve().parent / "static" / "owner-portfolio" / "index.html"
